module PDF417
  class LowLevelEncoder
    ...
    attr_reader :barcode_matrix, :writer

    def initialize(barcode_matrix)
      @barcode_matrix = barcode_matrix
      @writer = RowBuffer.new
    end

    def encode
      barcode_matrix.each_row do |codewords, cluster|
        writer.next_row
        writer << START_PATTERN
        codewords.each do |codeword|
          writer << CLUSTERS[cluster][codeword]
        end
        writer << STOP_PATTERN
      end
    ...
    # The low level bar-space patterns for codewords are represented below in
    # three mutually exclusive sets (clusters).
    # (0x15c70).to_s(2) => "10101110001110000"
    CLUSTERS = [
      [
